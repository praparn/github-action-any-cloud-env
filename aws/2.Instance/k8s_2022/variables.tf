...
variable "instances_per_az" {
  type = number
  default = 1
  description = "Multiple AZ will create with this number x 3. Ex: 1x3 = 3 instance at lease. Total number of instance per az to create on this task (3 - 9). In case you need to create with huge number. AWS will have service quota by default. So please check on KB: https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-resource-limits.html before operate"
  validation {
    condition     = var.instances_per_az >= 1 && var.instances_per_az <= 3 && floor(var.instances_per_az) == var.instances_per_az
    error_message = "Instances number per az is accept in range : 1-3."
  }
}
...
variable "associate_public_ip_address" {
  type = bool
  default = true
  description = "Need to assign public ip address (ENI) for this instance set to true for public access instance."
}
...
variable "volume_size_boot_std" {
  ...
  validation {
    condition     = var.volume_size_boot_std >= 10 && var.volume_size_boot_std <= 800 && floor(var.volume_size_boot_std) == var.volume_size_boot_std
    error_message = "Accepted disk size is : 10-800."
  }
}
